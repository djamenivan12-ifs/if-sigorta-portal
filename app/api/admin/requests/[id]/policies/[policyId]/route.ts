...
export async function GET(_request: Request, { params }: {
    ...
    const { id, policyId } = await params, db = createServiceClient();
    ...
    const { data: document, error: docError } = await db.from("insurance_policies").select("storage_path").eq("id", policyId).eq("request_id", id).maybeSingle();
