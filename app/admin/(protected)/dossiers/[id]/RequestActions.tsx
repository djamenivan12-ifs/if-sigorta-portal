...
type Action =
  "confirm_payment" | "reject_payment" | "start_policy" | "cancel_request";
...
export default function RequestActions({
  ...
  const [insurers, setInsurers] = useState<InsuranceOption[]>([]);
  ...
  async function performAction(
    ...
    try {
      ...
      if (!response.ok || !result.success) {
        throw new Error(result.error || "La modification du dossier a échoué.");
      }
  ...
  function rejectPayment() {
    ...
    void performAction("reject_payment", rejectionReason.trim());
  }
  ...
  async function openInsurerModal() {
    ...
    try {
      ...
      const result = (await response.json()) as InsuranceOptionsResponse;

      if (!response.ok || !result.success) {
        throw new Error(
          result.error || "Impossible de charger les assureurs disponibles.",
        );
      }
  ...
  function confirmInsurerSelection() {
    if (!selectedInsurerId) {
      setInsurerError("Veuillez sélectionner un assureur.");

      return;
    }

    void performAction("start_policy", undefined, selectedInsurerId);
  }

  const paymentCanBeReviewed = currentStatus === "payment_review";

  const policyCanStart = currentStatus === "payment_confirmed";

  const requestCanBeCancelled =
    currentStatus !== "policy_available" && currentStatus !== "cancelled";

  const noActionAvailable =
    !paymentCanBeReviewed && !policyCanStart && !requestCanBeCancelled;
          ...
          {paymentCanBeReviewed && (
            <>
              <button
                ...
                onClick={() => void performAction("confirm_payment")}
          ...
          >
            <div className="flex items-start justify-between gap-4">
              <div className="min-w-0">
                ...
                <p className="mt-2 text-sm leading-6 text-slate-500">
                  Sélectionnez l’assureur avant de commencer la préparation de
                  l’assurance.
                </p>
              </div>
              ...
              <button
                type="button"
                aria-label="Fermer"
                disabled={loadingAction === "start_policy"}
                onClick={closeInsurerModal}
              ...
              {insurersLoading ? (
                ...
                <div className="space-y-3">
                  {insurers.map((insurer) => {
                    const selected = selectedInsurerId === insurer.id;
                ...
                !insurerError && (
                  <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-4 text-sm leading-6 text-amber-800">
                    Aucun assureur ne possède actuellement un tarif applicable à
                    ce dossier.
                  </div>
                )
              )}
            ...
            <div className="mt-7 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
              ...
              <button
                ...
                onClick={confirmInsurerSelection}
