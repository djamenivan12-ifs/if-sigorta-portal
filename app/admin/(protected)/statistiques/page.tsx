import { windowFor } from "@/lib/dashboard/model";
import { cents } from "@/lib/accounting/model";
function paymentAmount(value: number | string | null) {
  const amount = cents(value);
  return amount === null ? NaN : amount / 100;
}
...
function calculateTrend(current: number, previous: number): TrendResult {
  if (
    ...
    return { value: NaN, direction: "stable" };
  const change = previous === 0 ? 0 : ((current - previous) / previous) * 100;
  return {
    value: Math.abs(change),
    direction: Math.abs(change) < 0.01 ? "stable" : change > 0 ? "up" : "down",
  };
}
...
function getTrendText(trend: TrendResult) {
  if (!Number.isFinite(trend.value)) return "Comparaison indisponible";
  ...
  return `${arrow} ${formatPercentage(trend.value)}`;
}

function getTrendClassName(trend: TrendResult) {
...
export default async function StatisticsPage() {
  ...
  const currentYear = Number(istanbulDate.slice(0, 4));

  const currentMonth = Number(istanbulDate.slice(5, 7));

  const monthStart = new Date(
    `${currentYear}-${String(currentMonth).padStart(2, "0")}-01T00:00:00+03:00`,
  );
  ...
  const previousMonthNumber = currentMonth === 1 ? 12 : currentMonth - 1;

  const previousMonthStart = new Date(
    `${previousMonthYear}-${String(previousMonthNumber).padStart(
      2,
      "0",
    )}-01T00:00:00+03:00`,
  );
  ...
  const [
    ...
    serviceClient.from("insurance_requests").select("id", {
      count: "exact",
      head: true,
    }),
    ...
    serviceClient
      ...
      .lt(
        "created_at",
        new Date(
          windowFor("month", now.toISOString()).previousEnd,
        ).toISOString(),
      ),
    ...
    readAll(
      serviceClient
        ...
        .lt("verified_at", nextMonthStart.toISOString())
        .order("id"),
    ),
    ...
    readAll(
      serviceClient
        ...
        .eq("status", "confirmed")
        .gte("verified_at", previousMonthStart.toISOString())
        .lt(
          "verified_at",
          new Date(
            windowFor("month", now.toISOString()).previousEnd,
    ...
    readAll(serviceClient.from("clients").select("nationality").order("id")),
    ...
    serviceClient
      ...
      .in("status", ["payment_confirmed", "policy_preparation"]),
  ...
  if (errors.length > 0) {
    throw new Error(
      errors.map((error) => error?.message ?? "Erreur Supabase").join(" | "),
    );
  }
  ...
  const totalPoliciesAvailable = totalPoliciesAvailableResult.count ?? 0;

  const totalConfirmedPayments = (totalPaymentsConfirmedResult.data ??
    []) as PaymentRow[];

  const totalRevenue = totalConfirmedPayments.reduce(
    (total, payment) => total + paymentAmount(payment.expected_amount),
    0,
  );

  const paymentsToReview = paymentsToReviewResult.count ?? 0;

  const policiesToPrepare = policiesToPrepareResult.count ?? 0;
  ...
  const currentMonthRequests = currentMonthRequestsResult.count ?? 0;

  const previousMonthRequests = previousMonthRequestsResult.count ?? 0;

  const oneYearRequests = oneYearRequestsResult.count ?? 0;

  const twoYearRequests = twoYearRequestsResult.count ?? 0;

  const currentMonthRejected = currentMonthRejectedResult.count ?? 0;

  const currentMonthAvailable = currentMonthAvailableResult.count ?? 0;
  ...
  const confirmedPaymentsThisMonth = currentMonthPayments.length;
  ...
  const nationalityRows = (nationalityClientsResult.data ??
    []) as NationalityRow[];
  ...
  const totalClientsWithNationality = Array.from(
    nationalityCounts.values(),
  ).reduce((total, count) => total + count, 0);

  const nationalityData = Array.from(nationalityCounts.entries())
    .map(([nationality, count]) => ({
      ...
      percentage:
        totalClientsWithNationality > 0
          ? (count / totalClientsWithNationality) * 100
          : 0,
    }))
  ...
  const directRequests = directRequestsResult.count ?? 0;

  const partnerRequests = partnerRequestsCountResult.count ?? 0;

  const directMonthRequests = directMonthRequestsResult.count ?? 0;

  const partnerMonthRequests = partnerMonthRequestsResult.count ?? 0;

  const directAvailable = directAvailableResult.count ?? 0;

  const partnerAvailable = partnerAvailableResult.count ?? 0;

  const confirmedPaymentsBySource = (confirmedPaymentsBySourceResult.data ??
  ...
  confirmedPaymentsBySource.forEach((payment) => {
    const requestRelation = normalizeRelation(payment.request);

    const amount = paymentAmount(payment.expected_amount);

    if (requestRelation?.source === "partner") {
      partnerRevenue += amount;

      if (requestRelation.partner_id) {
        const previous = partnerRevenueMap.get(requestRelation.partner_id) ?? {
          revenue: 0,
          confirmedPayments: 0,
        };

        partnerRevenueMap.set(requestRelation.partner_id, {
          revenue: previous.revenue + amount,

          confirmedPayments: previous.confirmedPayments + 1,
        });
      }
    ...
    if (requestRelation?.source === "direct") {
      directRevenue += amount;
    }
  });

  const partnerRequestRows = (partnerRequestsResult.data ??
    []) as PartnerRequestRow[];

  const partnerRequestMap = new Map<
    string,
    {
      requests: number;
      availablePolicies: number;
  ...
  partnerRequestRows.forEach((requestRow) => {
    if (!requestRow.partner_id) {
      return;
    }

    const previous = partnerRequestMap.get(requestRow.partner_id) ?? {
      requests: 0,
      availablePolicies: 0,
    };

    partnerRequestMap.set(requestRow.partner_id, {
      requests: previous.requests + 1,

      availablePolicies:
        previous.availablePolicies +
        (requestRow.status === "policy_available" ? 1 : 0),
    });
  });

  const partners = (partnersResult.data ?? []) as PartnerRow[];

  const partnerPerformance: PartnerPerformance[] = partners
    .map((partner) => {
      const requestStats = partnerRequestMap.get(partner.id) ?? {
        requests: 0,
        availablePolicies: 0,
      };

      const paymentStats = partnerRevenueMap.get(partner.id) ?? {
        revenue: 0,
        confirmedPayments: 0,
      };
      ...
      return {
        ...
        availablePolicies: requestStats.availablePolicies,

        confirmedPayments: paymentStats.confirmedPayments,
  ...
  const partnerShare =
    totalRequests > 0 ? (partnerRequests / totalRequests) * 100 : 0;
  ...
  return (
    <PageFrame
      ...
      detail={false}
      sections={[
        { id: "section-1", label: "Vue globale — historique" },
        { id: "section-2", label: "Origine des dossiers" },
        { id: "section-3", label: "Performance des partenaires" },
        { id: "section-4", label: "Comparaison mensuelle" },
        { id: "section-5", label: "Performance des agents" },
      ]}
    >
      <div className="mx-auto w-full min-w-0 max-w-[1500px]">
          ...
          <div className="flex min-w-0 flex-col gap-4 sm:gap-5 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-3xl text-[13px] leading-6 text-slate-500 sm:mt-3 sm:text-sm sm:leading-7 lg:text-base">
                Analyse globale de l’activité IF Sigorta, des clients directs et
                des partenaires.
              </p>
        ...
        <p className="my-4 text-sm leading-6 text-slate-600">
          Les totaux historiques couvrent tous les dossiers. Le mois courant est
          comparé à la même durée écoulée du mois précédent. Les encaissements
          correspondent aux paiements confirmés ; les polices disponibles
          décrivent l’état actuel des dossiers.
        </p>
        ...
        <section className="mt-4 sm:mt-6">
          ...
          <div className="grid min-w-0 grid-cols-2 gap-3 sm:gap-4 xl:grid-cols-4">
            ...
            <StatCard
              ...
              value={totalPoliciesAvailable.toLocaleString("fr-FR")}
            ...
            <StatCard
              ...
              value={totalConfirmedPayments.length.toLocaleString("fr-FR")}
        ...
        <section className="mt-4 grid min-w-0 grid-cols-1 gap-3 sm:mt-6 md:grid-cols-2 sm:gap-4">
          ...
          >
            ...
            <p className="mt-3 text-3xl font-semibold tracking-[-0.04em] text-orange-900">
              {paymentsToReview.toLocaleString("fr-FR")}
            </p>

            <p className="mt-2 text-sm text-orange-700">
              paiement
              {paymentsToReview !== 1 ? "s" : ""} à vérifier
            </p>
          </Link>
          ...
          >
            ...
            <p className="mt-3 text-3xl font-semibold tracking-[-0.04em] text-[#102B20]">
              {policiesToPrepare.toLocaleString("fr-FR")}
            </p>

            <p className="mt-2 text-sm text-[#31513B]">
              police
              {policiesToPrepare !== 1 ? "s" : ""} à préparer
            </p>
          </Link>
        </section>
        ...
        <section className="mt-4 min-w-0 rounded-2xl border border-slate-200/80 bg-white p-4 sm:mt-6 sm:rounded-[1.5rem] sm:p-6">
          <div className="flex min-w-0 flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <h2
                className="mt-2 break-words text-lg font-semibold tracking-[-0.02em] text-[#102B20] sm:text-xl"
                id="section-2"
              >
                Origine des dossiers
              </h2>
              ...
              <p className="mt-2 text-sm text-slate-500">
                Comparaison de l’activité générée directement et par les
                partenaires.
              </p>
            </div>
            ...
            <div className="w-full min-w-0 rounded-xl bg-[#EEF6EC] px-3 py-3 sm:w-auto sm:px-4">
              ...
              <p className="mt-1 text-xl font-black text-[#0B5D3B]">
                {formatPercentage(partnerShare)}
              </p>
            </div>
          </div>

          <div className="mt-4 min-w-0 overflow-x-auto sm:mt-6">
            <table className="min-w-full" aria-label="Statistiques">
              <thead>
              ...
              <tbody>
                <SourceRow
                  label="Dossiers"
                  direct={directRequests.toLocaleString("fr-FR")}
                  partner={partnerRequests.toLocaleString("fr-FR")}
                  total={totalRequests.toLocaleString("fr-FR")}
                />

                <SourceRow
                  label="Dossiers ce mois"
                  direct={directMonthRequests.toLocaleString("fr-FR")}
                  partner={partnerMonthRequests.toLocaleString("fr-FR")}
                  total={currentMonthRequests.toLocaleString("fr-FR")}
                />

                <SourceRow
                  label="Assurances disponibles"
                  direct={directAvailable.toLocaleString("fr-FR")}
                  partner={partnerAvailable.toLocaleString("fr-FR")}
                  total={totalPoliciesAvailable.toLocaleString("fr-FR")}
                />

                <SourceRow
                  label="Encaissements confirmés"
                  direct={formatCurrency(directRevenue)}
                  partner={formatCurrency(partnerRevenue)}
        ...
        <section className="mt-4 min-w-0 rounded-2xl border border-slate-200/80 bg-white p-4 sm:mt-6 sm:rounded-[1.5rem] sm:p-6">
          <div className="flex min-w-0 flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p className="mt-2 text-sm text-slate-500">
                Classement par chiffre d’affaires confirmé puis par nombre de
                dossiers.
              </p>
            </div>
          ...
          {partnerPerformance.length === 0 ? (
            <div className="mt-4 rounded-2xl border border-dashed border-slate-200 bg-[#FAFCFA] p-5 text-center sm:mt-6 sm:p-8">
              ...
              <p className="mt-2 text-sm text-slate-500">
                Les performances apparaîtront ici après la création des
                partenaires.
              </p>
            </div>
          ) : (
        ...
        <section className="mt-4 min-w-0 rounded-2xl border border-slate-200/80 bg-white p-4 sm:mt-6 sm:rounded-[1.5rem] sm:p-6">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-500">
              Comparaison sur une durée écoulée équivalente.
            </p>
          </div>
          ...
          <div className="mt-4 grid min-w-0 grid-cols-1 gap-3 sm:mt-6 md:grid-cols-2 sm:gap-4">
            <div className="min-w-0 rounded-xl border border-slate-100 bg-[#FAFCFA] p-4 sm:rounded-2xl sm:p-5">
              ...
              <p className="mt-2 break-words text-2xl font-semibold tracking-[-0.04em] text-[#102B20] sm:text-3xl lg:text-4xl">
                {currentMonthRequests.toLocaleString("fr-FR")}
              </p>
              ...
              <div className="mt-3 flex min-w-0 flex-col items-start gap-2 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
                <span className="text-sm text-slate-500">
                  Mois précédent :{" "}
                  {previousMonthRequests.toLocaleString("fr-FR")}
                </span>
            ...
            <div className="min-w-0 rounded-xl border border-slate-100 bg-[#FAFCFA] p-4 sm:rounded-2xl sm:p-5">
              <p className="text-sm font-medium text-slate-500">
                Encaissements confirmés du mois
              </p>
              ...
              <div className="mt-3 flex min-w-0 flex-col items-start gap-2 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
                <span className="text-sm text-slate-500">
                  Mois précédent : {formatCurrency(previousMonthRevenue)}
                </span>
        ...
        <div className="mt-4 sm:mt-6">
          <MonthlyPerformance
            ...
            availablePolicies={currentMonthAvailable}
            rejectedPayments={currentMonthRejected}
            confirmedPayments={confirmedPaymentsThisMonth}
            revenue={currentMonthRevenue}
            oneYearRequests={oneYearRequests}
            twoYearRequests={twoYearRequests}
          />
        </div>
        ...
        <div className="mt-4 grid min-w-0 gap-4 sm:mt-6 sm:gap-5 xl:grid-cols-2">
          <NationalityStats
            ...
            total={totalClientsWithNationality}
          />
        ...
        <section className="mt-4 min-w-0 rounded-2xl border border-slate-200/80 bg-white p-4 sm:mt-6 sm:rounded-[1.5rem] sm:p-6">
          <div className="flex min-w-0 flex-col gap-4 sm:flex-row sm:items-center sm:justify-between sm:gap-5">
            <div>
              ...
              <p className="mt-2 text-sm text-slate-500">
                Consultez la charge de travail, les délais et le taux de
                finalisation de chaque agent.
              </p>
            </div>
