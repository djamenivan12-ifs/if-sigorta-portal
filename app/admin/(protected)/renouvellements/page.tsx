import { daysRemaining } from "@/lib/dashboard/model";
...
type RequestRelation = {
  ...
  client: ClientRelation | ClientRelation[] | null;
};
...
type RenewalPriority = "critical" | "high" | "medium";
...
function unwrapRequest(relation: RequestRelation | RequestRelation[] | null) {
...
function unwrapClient(relation: ClientRelation | ClientRelation[] | null) {
  if (Array.isArray(relation)) {
    return relation[0] ?? null;
  }
...
function getDaysRemaining(policyEndDate: string) {
  return daysRemaining(policyEndDate, new Date().toISOString());
}
...
function getDaysText(daysRemaining: number) {
  if (daysRemaining < 0) {
    ...
    return `Expirée depuis ${days} jour${days !== 1 ? "s" : ""}`;
  }
...
function getWhatsappMessage({
  ...
  const dateLabel = formatDate(policyEndDate);

  let remainingText = "";

  if (daysRemaining > 0) {
    remainingText = `Il vous reste ${daysRemaining} jour${
      daysRemaining !== 1 ? "s" : ""
    } avant l'expiration.`;
  } else if (daysRemaining === 0) {
    remainingText = "Votre assurance expire aujourd’hui.";
  } else {
    const days = Math.abs(daysRemaining);

    remainingText = `Votre assurance est expirée depuis ${days} jour${
      days !== 1 ? "s" : ""
    }.`;
  }
...
export default async function RenewalsPage({
  ...
  const renewals: RenewalView[] = [];

  for (const row of rows) {
    const request = unwrapRequest(row.request);

    if (!request || !request.policy_end_date) {
      continue;
    }
    ...
    const whatsapp = client
      ? `${client.whatsapp_country_code ?? ""}${client.whatsapp_number ?? ""}`.replace(
          /\D/g,
          "",
        )
      : "";
  ...
  renewals.sort((first, second) => first.daysRemaining - second.daysRemaining);

  const expiredCount = renewals.filter((item) => item.daysRemaining < 0).length;
  ...
  const upcomingCount = renewals.filter(
    (item) => item.daysRemaining > 7 && item.daysRemaining <= 30,
  ).length;

  const listParams = await searchParams;
  const listQuery = scalar(listParams.q);
  const listFilter = scalar(listParams.filter);
  const filteredRows = renewals.filter(
    (row) =>
      matchesSearch([row.clientName, row.requestCode], listQuery) &&
      (!listFilter || row.status === listFilter),
  );
    ...
    >
      <div className="mx-auto w-full min-w-0 max-w-[1500px]">
          ...
          <div className="flex min-w-0 flex-col gap-4 sm:gap-5 lg:flex-row lg:items-center lg:justify-between">
            <div className="min-w-0">
              ...
              <p className="mt-2 max-w-3xl text-[13px] leading-6 text-slate-500 sm:mt-3 sm:text-sm sm:leading-7 lg:text-base">
                Assurances arrivant à expiration dans les 30 prochains jours ou
                déjà expirées.
              </p>
            </div>
        ...
        <section className="mt-4 grid min-w-0 grid-cols-2 gap-3 sm:mt-6 sm:gap-4 xl:grid-cols-4">
          <SummaryCard
            label="À contacter"
            value={renewals.filter((item) => item.status === "pending").length}
                    ...
                    <div className="flex min-w-0 flex-col gap-5 lg:flex-row lg:items-center lg:justify-between lg:gap-6">
                        ...
                        <div className="flex min-w-0 flex-wrap items-center gap-2">
                          ...
                          <span
                            className={`inline-flex max-w-full rounded-full px-2.5 py-1 text-[10px] font-bold sm:px-3 sm:text-xs ${
                              getStatusInfo(renewal.status).className
                            }`}
                          >
                            {getStatusInfo(renewal.status).label}
                          </span>
                        ...
                        <div className="mt-4 grid min-w-0 grid-cols-1 gap-3 text-[12px] text-slate-600 sm:grid-cols-2 sm:text-sm">
                          <div className="min-w-0 rounded-xl bg-slate-50 p-3 sm:bg-transparent sm:p-0">
                            ...
                            <span className="break-words">
                              {formatDate(renewal.policyEndDate)}
                            </span>
                          </div>
                          ...
                          <div className="min-w-0 rounded-xl bg-slate-50 p-3 sm:bg-transparent sm:p-0">
                            ...
                            <span className="break-words">
                              {getDaysText(renewal.daysRemaining)}
                            </span>
                          </div>
                          ...
                          <div className="min-w-0 rounded-xl bg-slate-50 p-3 sm:bg-transparent sm:p-0">
                            ...
                            <span className="break-all">
                              {renewal.whatsapp || "Non renseigné"}
                            </span>
                          </div>
                      ...
                      <div className="flex w-full min-w-0 shrink-0 flex-col gap-2 lg:w-60">
                        ...
                        {renewal.status !== "interested" && (
                          <RenewalInterestButton renewalId={renewal.id} />
                        )}
