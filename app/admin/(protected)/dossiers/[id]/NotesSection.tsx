...
export default function NotesSection({ requestId, notes }: NotesSectionProps) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!trimmedContent) {
      setErrorMessage("Écrivez une note avant de l’ajouter.");
    ...
    try {
      ...
      if (!response.ok || !result.success) {
        throw new Error(result.error || "Impossible d’ajouter la note.");
      }
    ...
    <section className="rounded-[1.5rem] border border-slate-200/80 bg-white p-5 sm:p-6">
      ...
      <div className="mt-8 border-t border-slate-100 pt-6">
        <div className="flex items-center justify-between">
          <h3 className="font-semibold text-[#102B20]">Notes enregistrées</h3>
